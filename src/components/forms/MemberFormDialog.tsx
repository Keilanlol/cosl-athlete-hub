  ...
  const onSubmit = async (values: FormValues) => {
    ...
    try {
      ...
      if (editing) {
        const { error } =
          kind === "fed"
            ? await supabase.from("federation_members").update(payload).eq("id", editing.id)
            : await supabase.from("club_members").update(payload).eq("id", editing.id);
        ...
        const { data, error } =
          kind === "fed"
            ? await supabase.from("federation_members").insert({ ...insertPayload, federation_id: orgId }).select("id").single()
            : await supabase.from("club_members").insert({ ...insertPayload, club_id: orgId }).select("id").single();
        if (error) throw error;
            ...
            {!editing && (
              <div className="space-y-1.5 rounded-md border border-dashed border-border bg-muted/40 p-3">
                ...
                {selectedPersonId && (
                  <button
                    type="button"
                    onClick={() => setValue("personId", "", { shouldValidate: true })}
            ...
            <Controller
              ...
              render={({ field: f }) => (
                <div className="flex items-center justify-between rounded-md border border-border px-3 py-2">
                  <Label htmlFor={f.name} className="cursor-pointer">Membre actif</Label>
